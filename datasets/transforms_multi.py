...
def hflip(clip, target):
    ...
    if "sub_boxes" in target and len(target["sub_boxes"]) > 0:
        sub_boxes = target["sub_boxes"]
        target["sub_boxes"] = sub_boxes[:, [2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) + torch.as_tensor([w, 0, w, 0])

    if "obj_boxes" in target and len(target["obj_boxes"]) > 0:
        obj_boxes = target["obj_boxes"]
        target["obj_boxes"] = obj_boxes[:, [2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) + torch.as_tensor([w, 0, w, 0])
...
def vflip(image,target):
    ...
    if "sub_boxes" in target and len(target["sub_boxes"]) > 0:
        sub_boxes = target["sub_boxes"]
        target["sub_boxes"] = sub_boxes[:, [0, 3, 2, 1]] * torch.as_tensor([1, -1, 1, -1]) + torch.as_tensor([0, h, 0, h])

    if "obj_boxes" in target and len(target["obj_boxes"]) > 0:
        obj_boxes = target["obj_boxes"]
        target["obj_boxes"] = obj_boxes[:, [0, 3, 2, 1]] * torch.as_tensor([1, -1, 1, -1]) + torch.as_tensor([0, h, 0, h])
...
def resize(clip, target, size, max_size=None):
    ...
    if "sub_boxes" in target and len(target["sub_boxes"]) > 0:
        target["sub_boxes"] = target["sub_boxes"] * torch.as_tensor([ratio_width, ratio_height, ratio_width, ratio_height])

    if "obj_boxes" in target and len(target["obj_boxes"]) > 0:
        target["obj_boxes"] = target["obj_boxes"] * torch.as_tensor([ratio_width, ratio_height, ratio_width, ratio_height])
...
class Normalize(object):
    ...
    def __call__(self, clip, target=None):
        ...
        if "sub_boxes" in target and len(target["sub_boxes"]) > 0:
            sub_boxes = box_xyxy_to_cxcywh(target["sub_boxes"])
            target["sub_boxes"] = sub_boxes / torch.tensor([w, h, w, h], dtype=torch.float32)

        if "obj_boxes" in target and len(target["obj_boxes"]) > 0:
            obj_boxes = box_xyxy_to_cxcywh(target["obj_boxes"])
            target["obj_boxes"] = obj_boxes / torch.tensor([w, h, w, h], dtype=torch.float32)
